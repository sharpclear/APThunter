...
async def get_available_models(
    ...
    username = None
    category_filter = category if category in {"malicious", "impersonation", "dga", "history_similarity", "apt_template_nrd"} else None
...
async def get_market_models(
    ...
    如果指定category，只返回该类型的模型（malicious/impersonation/dga/history_similarity/apt_template_nrd）
    """
    user_id = get_current_user_id(request, authorization)
    ...
    if category and category in ["malicious", "impersonation", "dga", "history_similarity", "apt_template_nrd"]:
