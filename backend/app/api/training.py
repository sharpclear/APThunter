...
from main import engine, minio_client, MINIO_BUCKET
import utils_ML
...
# 模型保存目录（使用saved_model目录，与现有模型保持一致）
MODEL_SAVE_DIR = os.path.join(os.path.dirname(__file__), '..', 'models', 'saved_model')
...
def train_model_task(task_id: str, training_data_file_id: int, model_name: str, model_desc: str):
    ...
    try:
        ...
        # 9. 保存模型和scaler
        logger.info(f"任务 {task_id}: 开始保存模型")
        ...
        # 计算 models 目录的绝对路径
        models_base_dir = os.path.normpath(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'models')))
...
async def upload_training_data(
    ...
    if file_ext not in ["csv", "txt"]:
        raise HTTPException(status_code=400, detail="仅支持CSV/TXT格式的文件")
