...
DEFAULT_POSITIVES_PATH = SCRIPT_DIR / "dataset" / "history_data" / "训练黑数据.xlsx"
...
def prepare_positive_seeds(raw_domains: Iterable[Any]) -> list[PositiveSeed]:
    ...
    for raw in raw_domains:
        ...
        seen.add(domain)
        record = parse_domain(domain)
        route, reason = route_positive_domain(record)
        seeds.append(
            PositiveSeed(
...
def route_positive_domain(record: Any) -> tuple[str, str]:
...
def score_nrd_domains(
    ...
    *,
    ...
    for raw in raw_domains:
        ...
        record = parse_domain(domain)
        route, _ = route_positive_domain(record)
...
def write_candidates(path: Path, candidates: list[Candidate]) -> None:
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for candidate in candidates:
            ...
            for key in ("tfidf_similarity", "rerank_score", "final_score"):
