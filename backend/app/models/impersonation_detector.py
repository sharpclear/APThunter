...
try:
    from app.models.impersonation_opus_llm_judge import KEEP_RECOMMENDATIONS, MODEL_NAME as LLM_MODEL_NAME, build_candidate_items, judge_batch
...
TARGET_EXCEL_PATH = r"E:\项目\APT组织域名预测\预测结果\仿冒检测\目标域名\输入列表.xlsx"
...
LLM_BATCH_SIZE = int(os.getenv("IMPERSONATION_LLM_BATCH_SIZE", os.getenv("LLM_JUDGER_BATCH_SIZE", "200")))
LLM_MAX_WORKERS = int(os.getenv("IMPERSONATION_LLM_MAX_WORKERS", os.getenv("LLM_JUDGER_MAX_WORKERS", "10")))
LLM_TIMEOUT_SEC = int(os.getenv("IMPERSONATION_LLM_TIMEOUT_SEC", os.getenv("LLM_JUDGER_TIMEOUT_SEC", "180")))
LLM_MAX_RETRIES = int(os.getenv("IMPERSONATION_LLM_MAX_RETRIES", "3"))
LLM_RETRY_SLEEP_SEC = int(os.getenv("IMPERSONATION_LLM_RETRY_SLEEP_SEC", "5"))
LLM_MAX_TOKENS = int(os.getenv("IMPERSONATION_LLM_MAX_TOKENS", "32768"))
...
def infer_unit_type_column(df: pd.DataFrame) -> Optional[str]:
...
def infer_unit_subtype_column(df: pd.DataFrame) -> Optional[str]:
    for col in df.columns:
        name = str(col).strip().lower()
        if name in {
            "单位小类",
...
def infer_unit_subtype_label_column(df: pd.DataFrame) -> Optional[str]:
    for col in df.columns:
        name = str(col).strip().lower()
        if name in {
            "单位小类名称",
...
def _read_domains_from_table(file_content: bytes, filename: str) -> pd.DataFrame:
    ...
    if file_ext == "csv":
        ...
        for encoding in READ_ENCODINGS:
            try:
                return pd.read_csv(io.BytesIO(file_content), dtype=str, encoding=encoding)
...
def read_official_domains_from_file(file_content: bytes, filename: str) -> List[Tuple[str, str]]:
    try:
        ...
        if df.empty:
            return []
        domain_col, company_col = infer_domain_and_company_columns(df)
        type_col = infer_unit_type_column(df)
        subtype_col = infer_unit_subtype_column(df)
        subtype_label_col = infer_unit_subtype_label_column(df)
        official_domains = []
        for _, row in df.iterrows():
            domain = normalize_host(row.get(domain_col, ""))
            company = row.get(company_col, "") if company_col is not None else ""
            unit_type = row.get(type_col, "") if type_col is not None else ""
            unit_subtype = row.get(subtype_col, "") if subtype_col is not None else ""
            unit_subtype_label = row.get(subtype_label_col, "") if subtype_label_col is not None else ""
            if pd.isna(company):
                company = ""
            if pd.isna(unit_type):
                unit_type = ""
            if pd.isna(unit_subtype):
                unit_subtype = ""
            if pd.isna(unit_subtype_label):
                unit_subtype_label = ""
            if domain:
                official_domains.append((
                    ...
                    domain,
                    str(unit_type).strip(),
                    str(unit_subtype).strip(),
                    str(unit_subtype_label).strip(),
                ))
...
def _run_detection(
    ...
    statistics = {
        ...
        "impersonation": len(results),
        "benign": len(detection_domains) - len(results),
        "impersonation_rate": len(results) / len(detection_domains) * 100 if detection_domains else 0,
        "总域名数": len(detection_domains),
        "算法候选数": llm_candidates_count,
        "仿冒域名数": len(results),
        "正常域名数": len(detection_domains) - len(results),
        "仿冒域名占比": f"{len(results) / len(detection_domains) * 100:.2f}%" if detection_domains else "0.00%",
...
def _build_result_excel(results: List[MatchResult], statistics: dict) -> bytes:
    ...
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        ...
        stats_df = pd.DataFrame({
            "统计项": ["总域名数", "算法候选数", "仿冒域名数", "正常域名数", "仿冒域名占比", "LLM研判状态", "LLM研判模型", "LLM已研判数"],
            "数值": [
                statistics.get("total", 0),
                statistics.get("算法候选数", 0),
                statistics.get("impersonation", 0),
                statistics.get("benign", 0),
                f"{statistics.get('impersonation_rate', 0):.2f}%",
        ...
        df.to_excel(writer, sheet_name="仿冒域名列表", index=False)
...
def detect_impersonation_domains(
    ...
    return detect_impersonation_domains_v1(
        official_domains,
        detection_domains,
...
def predict_from_domains(
    ...
    return predict_from_domains_v1(
        official_domains,
        detection_domains,
        similarity_threshold=similarity_threshold,
    )
