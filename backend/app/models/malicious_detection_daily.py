...
def predict_malicious_domains_predict_with_confidence(
    domains: List[str], model_path: Optional[str] = None
) -> List[Dict]:
    """
    订阅检测与预警用：与非订阅任务一致，**预测标签仅由 model.predict 决定**；
    同时为每条样本写入 **恶意类置信度**（模型支持 predict_proba 时为正类概率列；
    否则恶意样本记 1.0、正常样本记 0.0，便于后续按阈值筛选）。
    """
    if not domains:
        return []
    ...
    malicious_prob_col: Optional[np.ndarray] = None
    if hasattr(model, "predict_proba"):
        probabilities = model.predict_proba(features_scaled)
        if probabilities is not None and getattr(probabilities, "shape", (0,))[1] >= 2:
            malicious_prob_col = probabilities[:, 1]
        elif probabilities is not None and getattr(probabilities, "shape", (0,))[1] == 1:
            malicious_prob_col = probabilities[:, 0]

    results: List[Dict] = []
    for i, domain in enumerate(domains):
        label = int(predictions[i])
        if malicious_prob_col is not None:
            conf = float(malicious_prob_col[i])
        else:
            conf = 1.0 if label == 1 else 0.0
        results.append(
            {
                "域名": domain,
                "预测标签": label,
...
def predict_from_domains_subscription(
    domains: List[str],
    ...
    """
    订阅专用：生成 Excel/统计，并返回完整逐条结果列表（含恶意概率），供预警按阈值二次筛选。
    """
    if not domains:
        raise ValueError("域名列表为空")
    results = predict_malicious_domains_predict_with_confidence(domains, model_path)
    excel_bytes, statistics = _generate_excel_and_stats(results, malicious_only=malicious_only)
    return excel_bytes, statistics, results
    ...
    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
        ...
        if malicious_count > 0:
            malicious_df = pd.DataFrame([
                r for r in results if r['预测标签'] == 1
            ])
...
def _generate_excel_and_stats(results: List[Dict], malicious_only: bool = False) -> Tuple[bytes, Dict]:
    ...
    statistics = {
        ...
        '总域名数': total,
        '恶意域名数量': malicious_count,
        '正常域名数': benign_count,
        '恶意域名占比': f"{malicious_count / total * 100:.2f}%" if total > 0 else "0.00%"
    }
    return excel_bytes, statistics
