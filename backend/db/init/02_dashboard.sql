...
CREATE TABLE IF NOT EXISTS apt_events (
  ...
  link TEXT NULL COMMENT '原报告链接',
  event_type ENUM('major','normal') NOT NULL DEFAULT 'normal' COMMENT '事件类型',
  threat_type VARCHAR(64) NOT NULL DEFAULT '未分类' COMMENT '事件业务类型',
  releasing_product VARCHAR(255) NULL COMMENT '报告发布单位/产品',
...
CREATE TABLE IF NOT EXISTS threat_trends (
  ...
  c2_communication_count INT DEFAULT 0 COMMENT 'C2通信数',
