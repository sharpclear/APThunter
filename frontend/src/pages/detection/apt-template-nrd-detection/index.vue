...
async function handleSubmit() {
  if (!selectedModel.value)
    return message.warning('请选择APT模板检测模型')
  ...
  try {
    ...
    message.success(`APT模板新注册域名检测任务已提交！task: ${json.task_id || ''}`)
    resetForm()
  }
  catch (e: any) {
    ...
    <a-row :gutter="[24, 24]">
      <a-col :xs="24" :xl="16">
        <a-card class="form-card" :bordered="false">
          <div class="card-header">
            <div class="card-title">创建APT模板新注册域名检测任务</div>
            <div class="card-subtitle">
              基于APT注册模板库匹配待检测域名，输出风险分、风险等级和命中原因。
            </div>
            <div class="header-tags">
              <span class="mini-tag">APT模板库</span>
      ...
      <a-col :xs="24" :xl="8">
        <a-card title="处理流程" class="guide-card" :bordered="false">
          <ul class="guide-list">
            <li><span class="dot">1</span><span>加载APT注册模板库并编译模板匹配规则。</span></li>
