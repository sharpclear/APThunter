...
async function fetchAvailableModels() {
  ...
  try {
    const resp = await fetch(`${API_BASE}/models/available?category=history_similarity`, {
      method: 'GET',
      headers: buildHeaders(),
    })
    const json = await resp.json().catch(() => null)
...
async function handleSubmit() {
  ...
  try {
    ...
    fd.append('minScore', String(minScore.value))
    if (dataSource.value === 'upload' && uploadFile.value)
    ...
    const resp = await fetch(`${API_BASE}/history-similarity-tasks`, {
      method: 'POST',
      body: fd,
      headers: buildHeaders(),
    })
      ...
      <a-col :xs="24" :xl="16">
        <a-card class="form-card" :bordered="false">
          ...
          <a-form layout="vertical" class="task-form">
            ...
            <div class="form-section">
              <div class="section-title">步骤 3：配置筛选阈值</div>
              <a-form-item label="最低综合相似度（0-100）">
                <div class="threshold-row">
                  <a-slider v-model:value="minScore" class="threshold-slider" :min="0" :max="100" />
