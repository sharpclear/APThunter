...
import TaskResultModal from './task-result-modal.vue'

interface TaskItem {
  id: string
  createdAt: string
  taskType: '恶意域名检测' | '恶意性检测' | '仿冒域名检测' | 'DGA域名检测' | '历史高度相似检测' | 'APT模板新注册域名检测' | '恶意IP检测' | string
  model: string
  dataSource: {
...
async function downloadResult(record: TaskItem) {
  ...
  try {
    loading.value = true
    const resp = await fetch(`${API_BASE}/tasks/${record.id}/download`, {
      method: 'GET',
      headers: buildHeaders(),
    })
    if (!resp.ok) {
...
function viewResult(record: TaskItem) {
  if (!record.resultFileKey) {
    return message.warning('该任务暂无可查看的结果')
  }
  currentTaskId.value = record.id
  resultModalVisible.value = true
}

function handleModalDownload(taskId: string) {
  const record = tableData.value.find(t => t.id === taskId)
  if (record) {
  ...
  <page-container>
    <a-card :bordered="false">
      ...
      <a-form layout="inline" style="margin-bottom: 12px;" @submit.prevent>
        <a-form-item label="任务类型">
          ...
          >
            ...
            <a-select-option value="APT模板新注册域名检测">
              APT模板新注册域名检测
            </a-select-option>
      ...
      >
        ...
        <a-table-column key="actions" title="操作" width="280">
          <template #default="{ record }">
            <a-space>
              <a-button
                size="small"
