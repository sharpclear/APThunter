...
async function fetchPublishableModels() {
  try {
    const resp = await fetch(`${API_BASE}/models/publishable`, {
      method: 'GET',
      headers: buildHeaders(),
    })
...
async function fetchMarketModels() {
  ...
  try {
    const url = new URL(`${API_BASE}/models/market`, window.location.origin)
    ...
    else if (filterCategory.value === '历史APT域名相似性检测') {
      url.searchParams.set('category', 'history_similarity')
    }
    else if (filterCategory.value === '模板化APT域名检测') {
...
async function confirmPublish() {
  ...
  try {
    const resp = await fetch(`${API_BASE}/models/${publishingModel.value.id}/publish`, {
      method: 'POST',
      headers: {
        ...buildHeaders(),
...
async function revokePublic(model: MyModel) {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const resp = await fetch(`${API_BASE}/models/${model.id}/unpublish`, {
          method: 'POST',
          headers: buildHeaders(),
        })
...
async function addToMyModels(m: MarketModel) {
  ...
  try {
    const resp = await fetch(`${API_BASE}/models/${m.id}/acquire`, {
      method: 'POST',
      headers: buildHeaders(),
    })
        ...
        <a-card title="筛选与排序" :bordered="false">
          <div class="side-block">
            ...
            <a-radio-group v-model:value="filterCategory" @change="fetchMarketModels">
              ...
              <a-radio-button value="历史APT域名相似性检测">
                历史APT域名相似性检测
              </a-radio-button>
              <a-radio-button value="模板化APT域名检测">
                模板化APT域名检测
              </a-radio-button>
